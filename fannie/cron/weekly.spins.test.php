...
$dataV = $sql->num_rows($dataR);
if ($dataV == 0)
	echo cron_msg("SPINS: ERROR: Query returned 0 results");
else
	echo cron_msg("SPINS: Successfully run query (".$dataV." results)");
...
if (!$conn_id or !$login_id){
	echo cron_msg("SPINS: ERROR: FTP connect failed!");
}
...
if (!$upload)
	echo cron_msg("SPINS: ERROR: FTP upload failed");
else
	echo cron_msg("SPINS: Sales data successfully uploaded");
